module.exports = function BooleanExpressionParser() {
  /**
  ...
  this.parse = function (infix) {
    var tokens = infix.replace(/\(/g, ' ( ').replace(/\)/g, ' ) ').trim().split(/\s+/);
    var exprs = [];
    var ops = [];

    tokens.forEach(function (token) {
      if (isOp(token)) {
        while (ops.length > 0 && isOp(peek(ops)) && (
