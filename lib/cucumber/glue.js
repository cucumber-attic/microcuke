...
module.exports = function Glue(stepDefinitions, hooks) {
  this.createTestCase = function (pickle) {
    ...
    var afterHookTestSteps = hooks.filter(function (hook) {
      return hook.scope === 'after'; // TODO: AND matches pickle's tags
    }).map(function (hook) {
      return hook.createTestStep(pickle);
    });
    ...
    var testSteps = [].concat(beforeHookTestSteps).concat(pickleTestSteps).concat(afterHookTestSteps);
